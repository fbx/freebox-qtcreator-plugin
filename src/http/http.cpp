...
Version::Version(const QString &str)
    : mValid(false)
{
    QRegularExpression ex(QStringLiteral("^HTTP/(\\d)\\.(\\d)$"));
	QRegularExpressionMatch match = ex.match(str);

    if (match.hasMatch()) {
        mValid = true;
        mMajor = match.captured(1).toUInt();
        mMinor = match.captured(2).toUInt();
    }
}

Version::operator QString() const
{
    return QStringLiteral("HTTP/%1.%2").arg(QString::number(mMajor)).arg(QString::number(mMinor));
}
