...
void Server::incomingConnection(qintptr handle)
{
    ...
    connect(client, &Client::readyRead, this, &Server::readClient);
    connect(client, &Client::disconnected, this, &Server::disconnectClient);
    client->setSocketDescriptor(handle);
}

void Server::reply(QTcpSocket *socket, const Version &version, const IResponse &response)
{
    QTextStream(socket) << version << " " << response << CRLF;
}

void Server::replyHeader(QTcpSocket *socket, const QString &name, const QString &value)
{
...
void Server::replyError(QTcpSocket *socket, const Version &version, const IResponse &response)
{
...
void Server::replyHeader(QTcpSocket *socket, const QString &name, unsigned int value)
{
    ...
    const QStringList &headers)
{
    ...
    qCDebug(fbxLog) << "GET" << uri << version;

    if (mPath == QDir()) {
        qCWarning(fbxLog) << "server path not set";
        return replyError(client, version, Reply::InternalServerError());
    }

    QUrl url(uri, QUrl::StrictMode);
    if (!url.isValid()) {
        qCWarning(fbxLog) << "invalid URI" << qPrintable(uri);
        return replyError(client, version, Reply::BadRequest());
    }

    if (!url.isRelative()) {
        qCWarning(fbxLog) << "unsupported URI" << qPrintable(uri);
        return replyError(client, version, Reply::BadRequest());
    }
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        qCWarning(fbxLog) << "failed to open file" << qUtf8Printable(file.fileName());
        return replyError(client, version, Reply::InternalServerError());
    }
