...
Search::Search(
    const QNetworkInterface &iface, const QHostAddress &addr, const QString &st, QObject *parent)
    : UdpSocket(parent)
    , mIface(iface)
    , mSt(st)
{
    if (!bind(addr)) {
        qCWarning(fbxLog) << "failed bind for SSDP search:" << errorString();
        return;
    }
    ...
    connect(&m_timeout, &QTimer::timeout, this, &Search::handleTimeout);
}
...
void Search::sendPacket()
{
    ...
    if (!mIface.isValid()) {
        qCWarning(fbxLog, "invalid interface");
        return;
    }

    if (mSt.isEmpty()) {
        qCWarning(fbxLog, "ST field is empty");
        return;
    }
    ...
    writeDatagram(ba, ba.size(), QHostAddress(QLatin1String(Constants::ADDR4)), Constants::PORT);
}
