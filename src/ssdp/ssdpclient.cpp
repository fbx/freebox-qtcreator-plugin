...
void Client::start(const QString &nt)
{
    ...
    for (UdpListener *l : mListeners) {
        connect(l, &UdpListener::datagramReceived, this, &Client::notifyMessageReceived);
    }

    for (Search *s : mSearches) {
        connect(s, &Search::datagramReceived, this, &Client::notifyMessageReceived);
    }
...
void Client::notifyMessageReceived(const QHostAddress &addr, const QByteArray &datagram)
{
