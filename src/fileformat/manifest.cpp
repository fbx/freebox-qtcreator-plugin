...
{
    return objectValue(entry, objectValue(QStringLiteral("entryPoints")))[QStringLiteral("file")]
        .toString();
}
...
{
    return objectValue(entry, objectValue(QStringLiteral("entryPoints")))[QStringLiteral(
                                                                              "urlHandler")]
        .isArray();
}
...
bool Manifest::checkRequiredProperty(QJsonObject &obj, const QString &key, const QString &where)
{
    MAN_ASSERT_ERR(
        obj.contains(key),
        return false,
        QObject::tr("Missing required value for key '%1.%2'").arg(where).arg(key));

    return true;
}

bool Manifest::checkNumberProperty(
    QJsonObject &obj, const QString &key, int min, int max, const QString &where)
{
    MAN_ASSERT_ERR(
        isNumberValue(key, obj),
        return false,
        QObject::tr("Property '%1.%2' must be a number").arg(where).arg(key));

    int intVal = intValue(key, obj);
    MAN_ASSERT_ERR(
        intVal >= min && intVal <= max,
        return false,
        QObject::tr("Property '%1.%2' must be between %3 and %4 included")
            .arg(where)
            .arg(key)
...
{
    MAN_ASSERT_ERR(
        isStringValue(key, obj),
        return false,
        QObject::tr("Property '%1.%2' must be a regexp string").arg(where).arg(key));

    QString val = stringValue(key, obj);

    QRegularExpression re(val);

    MAN_ASSERT_ERR(
        re.isValid(),
        return false,
        QObject::tr("Invalid regexp in property '%1.%2' (value is '%3')")
            .arg(where)
            .arg(key)
            .arg(val));
...
bool Manifest::checkStringProperty(
    QJsonObject &obj, const QString &key, const QString &regExp, const QString &where)
{
    if (!obj.contains(key))
        return true;
    ...
    MAN_ASSERT_ERR(
        isStringValue(key, obj),
        return false,
        QObject::tr("Property '%1.%2' must be a string").arg(where).arg(key));
    ...
    MAN_ASSERT_ERR(
        match.hasMatch(),
        return false,
        QObject::tr("Invalid characters in property '%1.%2' (value is '%3', regexp is /^%4$/i)")
            .arg(where)
            .arg(key)
            .arg(val)
            .arg(regExp));
...
bool Manifest::check()
{
    ...
    MAN_ASSERT(
        checkRequiredProperty(m_obj, QStringLiteral("identifier"), QStringLiteral("(root)")),
        return false);
    MAN_ASSERT(
        checkStringProperty(
            m_obj,
    ...
    for (quint16 lo = 0; lo < eps.size(); lo++) {
        ...
        MAN_ASSERT_ERR(
            entryPointsValueIsValid(key),
            return false,
            QObject::tr("Property 'entryPoints.%1' must be an object").arg(key));
        MAN_ASSERT_ERR(
            isEntryPointsFileValid(key),
            return false,
            QObject::tr("Property 'entryPoints.%1.file' must be an existing file").arg(key));
        ...
        MAN_ASSERT(
            checkRequiredProperty(
                epObj, QStringLiteral("file"), QString::fromUtf8("entryPoints.%1").arg(key)),
            return false);
        MAN_ASSERT(
            checkStringProperty(
                epObj,
                QStringLiteral("file"),
                QStringLiteral("[/\\w\\.\\+~-]+"),
                QString::fromUtf8("entryPoints.%1").arg(key)),
            return false);
        ...
        if (epObj.contains(QStringLiteral("uiFlavor"))) {
            MAN_ASSERT(
                checkStringProperty(
                    epObj,
                    QStringLiteral("uiFlavor"),
                    QStringLiteral("(multi|classic)"),
                    QString::fromUtf8("entryPoints.%1").arg(key)),
                return false);
        }
        ...
        if (epObj.contains(defaultKey)) {
            ...
            MAN_ASSERT_ERR(
                defaultVal.isBool(),
                return false,
                QObject::tr("Property 'entryPoints.%1.default' must be a bool").arg(key));
            MAN_ASSERT_ERR(
                !(defaultVal.toBool() && m_nbDefaults++),
                return false,
                QObject::tr("Only one property 'entryPoints.<string>.default' can be true"));
        ...
        if (epObj.contains(urlsKey)) {
            MAN_ASSERT_ERR(
                isEntryPointsUrlHandlerValid(key),
                return false,
                QObject::tr("Property 'entryPoints.%1.urlHandler' must be an array").arg(key));
            ...
            for (int urlsIndex = 0; urlsIndex < urlsArray.size(); ++urlsIndex) {
                ...
                MAN_ASSERT_ERR(
                    urlsArray[urlsIndex].isObject(),
                    return false,
                    QObject::tr("Property 'entryPoints.%1.urlHandler[%2]' must be an object")
                        .arg(key)
                        .arg(QString::number(urlsIndex)));

                QJsonObject urlsArrayVal = urlsArray[urlsIndex].toObject();
                QString where
                    = QString::fromUtf8("entryPoints.%1.urlHandler[%2]").arg(key).arg(urlsIndex);
                ...
                MAN_ASSERT_ERR(
                    seenPattern,
                    return false,
                    QObject::tr("Property 'entryPoints.%1.urlHandler[%2]' must contain one of "
                                "'urlPattern' or 'mimeTypePattern' regexps, or both.")
                        .arg(key)
                        .arg(QString::number(urlsIndex)));

                MAN_ASSERT(
                    checkRequiredProperty(urlsArrayVal, QStringLiteral("actionId"), where),
                    return false);
                MAN_ASSERT(
                    checkStringProperty(
                        urlsArrayVal, QStringLiteral("actionId"), QStringLiteral("\\w{1,40}"), where),
                    return false);

                MAN_ASSERT(
                    checkRequiredProperty(urlsArrayVal, QStringLiteral("actionLabel"), where),
                    return false);
                MAN_ASSERT(
                    checkStringProperty(
                        urlsArrayVal,
                        QStringLiteral("actionLabel"),
                        QStringLiteral("[\\w '\"\\.\\+\\-\\!\\?:]{1,40}"),
                        where),
                    return false);

                MAN_ASSERT(
                    checkRequiredProperty(urlsArrayVal, QStringLiteral("score"), where),
                    return false);
                MAN_ASSERT(
                    checkNumberProperty(urlsArrayVal, QStringLiteral("score"), 0, 1000, where),
                    return false);
            }
        }
    }
