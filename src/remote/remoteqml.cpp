...
void QmlRemote::startDone(const QJsonValue &rep)
{
    ...
    emit started(v["qml_port"].toInt(), v["stdout_port"].toInt(), v["stderr_port"].toInt());
    mCurrent = 0;
}
...
void QmlRemote::start(const QString &entry, quint16 port, bool wait)
{
    ...
    if (mAddress.isNull()) {
        qCWarning(fbxLog, "remote address is not set");
        return;
    }
    ...
    if (!socket.waitForConnected(3000)) {
        qCWarning(fbxLog) << "fail to connect to" << mAddress.toString();
        return;
    }
    ...
    connect(mCurrent, &Util::JsonRpcRequest::error, this, &QmlRemote::startError);
    connect(mCurrent, &Util::JsonRpcRequest::done, this, &QmlRemote::startDone);
}
