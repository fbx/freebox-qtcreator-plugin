...
{
    return QLatin1String(":/freebox/templates/qml/");
}
...
void App::setProjectNameAndBaseDirectory(const QString &projectName,
                                         const QString &projectBaseDirectory)
{
    m_projectBaseDirectory = projectBaseDirectory;
    m_projectName = projectName.trimmed();
}

QString App::projectDirectory() const
{
    return QDir::cleanPath(m_projectBaseDirectory + QLatin1Char('/') + m_projectName);
}

QString App::projectName() const
{
    return m_projectName;
}

void App::setTemplateInfo(const TemplateInfo &templateInfo)
{
    m_templateInfo = templateInfo;
}

QString App::creatorFileName() const
{
    return m_creatorFileName;
}

QString App::templateDirectory() const
{
...
static QStringList templateNames()
{
    ...
    for (int i = 0; i < templateNameList.size() / 2; i++)
        templateNameList.swap(i, templateNameList.size() - i - 1);
...
QList<TemplateInfo> App::templateInfos()
{
...
QByteArray App::readFile(const QString &filePath, bool &ok) const
{
...
QString App::readAndAdaptTemplateFile(const QString &filePath, bool &ok) const
{
...
bool App::addTemplate(const QString &sourceDirectory,
...
bool App::addBinaryFile(const QString &sourceDirectory,
                        const QString &templateFileName,
                        const QString &tragetDirectory,
                        const QString &targetFileName,
                        Core::GeneratedFiles *files,
...
QString App::renameQmlFile(const QString &fileName)
{
    return fileName;
}

Core::GeneratedFiles App::generateFiles(QString *errorMessage)
{
    Core::GeneratedFiles files;
    ...
    foreach (const QFileInfo &templateFile, templateFiles) {
        ...
        m_creatorFileName = Core::BaseFileWizardFactory::buildFileName(projectDirectory(),
                                                                       projectName(),
                                                                       QLatin1String("pro"));
        } else  if (templateFile.fileName() == QLatin1String("main.fbxproject")) {
            targetFileName = projectName() + QLatin1String(".fbxproject");
        m_creatorFileName = Core::BaseFileWizardFactory::buildFileName(projectDirectory(),
                                                                       projectName(),
                                                                       QLatin1String("fbxproject"));
...
void App::addReplacementVariable(const QString &name, const QString &value)
{
    m_replacementVariables.insert(name, value);
}

void App::setReplacementVariables()
{
    addReplacementVariable(QLatin1String("projectName"), projectName());
}
